...
{synoptline}
{p2coldent :* {helpb xtdpdserial}}perform tests for autocorrelated residuals{p_end}
{synopt:{helpb xtdpdgmm postestimation##estat:estat serial}}perform Arellano-Bond test for autocorrelated residuals{p_end}
...
{p2coldent:* {opt u}}calculate the group-specific error component{p_end}
...
{synoptline}
{syntab:Options}
{synopt:{opt nogen:erate}}do not generate new variables{p_end}
{synoptline}
{p2colreset}{...}
...
{pstd}
{cmd:estat serial} reports the Arellano and Bond (1991) test for absence of autocorrelation in the first-differenced residuals at specific orders. For more powerful serial correlation tests, see {helpb xtdpdserial} (if installed).
...
{phang}
{opth order(numlist)} with {cmd:estat serial} specifies the orders of serial correlation to be tested for the first-differenced residuals. The default is {cmd:order(1 2)}.
...
{pstd}
Tests for absence of serial correlation in first differences can suffer from low power to detect deviations from the null hypothesis, especially when the true process is close to a random walk.
Alternative tests proposed by Yamagata (2008), Jochmans (2020), and Kripfganz, Demetrescu, and Hosseinkouchack (2025) can be more powerful. These are implemented in the command {helpb xtdpdserial},
which can be used as a postestimation command for {helpb xtdpdgmm} but needs to be installed separately.
...
{phang}
Kripfganz, S., M. Demetrescu, and M. Hosseinkouchack. 2025.
