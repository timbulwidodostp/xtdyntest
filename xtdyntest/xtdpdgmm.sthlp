...
{synopt:{opt f:irst}{cmd:(}{it:{help xtdpdgmm##options_spec:first_spec}}{cmd:)}}specify first-step estimates{p_end}
...
{synoptline}
{p2colreset}{...}
{p 4 6 2}* You can specify at most one of these options. {cmd:onestep} is the default unless option {opt nl(nl_spec)} or {opt first(first_spec)} is specified, in which case {cmd:twostep} is the default.{p_end}
...
{p 8 12 2}
[{it:name}] [{cmd:,} {opt nocons:tant} {opt copy}]
...
{phang}
{cmd:wmatrix(}[{it:wmat_type}] [{it:varname}] [{cmd:,} {opt r:atio(#)}]{cmd:)} specifies the weighting matrix to be used to obtain one-step GMM estimates or initial estimates for two-step or iterated GMM estimation.
{it:wmat_type} is either {opt un:adjusted}, {opt ind:ependent}, {opt sep:arate}, {opt identity}, or {opt r:esiduals}. The latter requires specification of {it:varname}.
...
{phang}
{cmd:first(}[{it:name}] [{cmd:,} {opt nocons:tant} {opt copy}]{cmd:)} requests to compute an optimal weighting matrix from first-step estimation results stored under the name {it:name}.
Suboption {opt noconstant} is required if there is a constant term in the second-step estimation but not in the first-step estimation. Suboption {opt copy} is required if the coefficient names of the first-step and second-step
estimations do not coincide; it is then the user's responsibility to ensure that the respective coefficients correspond to each other in the correct order.
{cmd:first(.)} or {cmd:first(, copy)} without an estimation name requests to use the previous estimation results currently in memory; some postestimation statistics may not be available in this case.
...
{phang2}{help xtdpdgmm##remarks_wmatrix:Initial weighting matrix}{p_end}
...
In that case, the nonlinear moment conditions added by the option {cmd:nl(noserial)} only require the mild additional assumption that both the group-specific error component and the initial observations of {it:depvar} are uncorrelated
with the first-differenced idiosyncratic error component. The moment conditions added by the option {cmd:nl(iid)} require an additional homoskedasticity assumption.
...
{marker remarks_wmatrix}{...}
{title:Initial weighting matrix}

{pstd}
The one-step GMM estimator is generally asymptotically inefficient unless all moment conditions refer to a transformed model and the idiosyncratic error component is homoskedastic and serially uncorrelated (in levels), or if the model is
just-identified with as many instruments as regression coefficients. If those conditions are not satisfied, the two-step GMM estimator can be applied, which uses an optimal weighting matrix based on the estimated first-step residuals.
While asymptotically the choice of the initial weighting matrix for the first-step estimation is innocuous for the two-step estimator, it can have substantial effects on the estimates in finite samples. To avoid this sensitivity to the
initial weighting matrix, the iterated GMM estimator can be used, which iterates between the estimation of the coefficients and the optimal weighting matrix until convergence.

{pstd}
A few choices for the initial weighting matrix have been proposed. The default option, {cmd:wmatrix(unadjusted)}, accounts for the first-order serial correlation in the first-differenced model and the correlation between the moment
conditions specified for different model transformations. The corresponding one-step estimator estimator equals the two-stage least squares estimator. Alternatively, {cmd:wmatrix(independent)} imposes a block-diagonal weighting matrix
that ignores the correlation across model transformations. {cmd:wmatrix(separate)} additionally ignores the serial correlation in the transformed errors.

{pstd}
In most cases, it is not recommended to use {cmd:wmatrix(independent)} or {cmd:wmatrix(separate)} unless there are compelling reasons to do so. These options are primarily provided to enable reproducibility of the results with some other
commands. {cmd:wmatrix(identity)} is even less recommended, as it completely ignores the data.

{pstd}
When some of the moment conditions refer to the untransformed model in levels, the optimal weighting matrix for the two-step estimator might be poorly estimated when the variance of the group-specific error component is large relative to
the variance of the idiosyncratic error component. To circumvent this problem, it can be useful to compute the optimal weighting matrix from residuals of another estimation which is free of such level moment conditions. For example, a
one-step GMM estimator (or even an alternative estimator implemented in a different command) based on a model transformation could be used initially to obtain the first-step residuals.
Option {opt first()} can be used to specify the respective first-step estimates. This procedure was suggested by Kruiniger (2022).

{pstd}
Alternatively, if an initial guess or estimate of the variance ratio of the group-specific error component to the idiosyncratic error component is available, the initial weighting matrix can be improved by providing a value for this
variance ratio with suboption {opt ratio(#)}. If this is a consistent estimate of the variance ratio and the errors are homoskedastic and serially uncorrelated, the one-step estimator can be asymptotically efficient even if it involves
moment conditions for the untransformed model.

{pstd}
An initial weighting matrix is not needed for the continuously-updating GMM estimator because it estimates the coefficients jointly with the optimal weighting matrix.
...
{it:Econometric Reviews} 41: 117-146.
