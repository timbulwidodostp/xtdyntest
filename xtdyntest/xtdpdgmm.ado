...
program define xtdpdgmm_gmm, eclass
	...
	*** type of weighting matrix ***
	...
	if `"`first'"' != "" {
		if "`wmatrix'" != "" {
			di as err "options wmatrix() and first() may not be combined"
			exit 184
		}
		if "`cugmm'" != "" {
			di as err "options cugmm() and first() may not be combined"
			exit 184
		}
		if "`onestep'" != "" {
			di as err "options onestep and first() may not be combined"
			exit 184
		}
		gettoken fname foptions : first , parse(",")
		if "`fname'" == "." {
			loc fname			""
		}
		if "`foptions'" == "" & "`fname'" != "," {
		...
		xtdpdgmm_parse_first `fname' `foptions' mopt(`mopt') regnames(`regnames')
	}
	...
	*** type of variance-covariance matrices ***
	if `"`vce'"' != "" {
		...
		if "`vce'" == "robust" {
			...
			if "`vcec'" == "DC" {
				if "`wmatrix'" != "unadjusted" & "`overid'" != "" {
					di as err "option vce(robust, dc) not available in combination with options wmatrix(`wmatrix') and overid"
					exit 184
				}
				if "`first'" != "" & "`twostep'" != "" {
					di as err "option vce(robust, dc) not available in combination with options first() and twostep"
					exit 184
				}
				if "`cugmm'" != "" {
					loc vcec			""
...
// 		}
	...
	*** current estimation results ***
	...
	if "`first'" == "" {
		eret loc wmatrix	"`wmatrix'"
	}
	else {
		eret loc wmatrix	"first `first'"
	}
...
program define xtdpdgmm_init, sclass
	...
	sret loc options	`"`nl' `onestep'`twostep'`igmm'`cugmm' `first' `options'"'
end
...
program define xtdpdgmm_parse_wmatrix, sclass
	...
	if `: word count `anything'' >= 2 {
		gettoken anything resid : anything
	}
	loc length			: length loc anything
	...
	sret loc resid		"`resid'"
	sret loc ratio		= `ratio'
end

*==================================================*
**** syntax parsing for the first-step estimates ****
program define xtdpdgmm_parse_first, sclass
	version 13.0
	syntax [name] [, noCONStant COPY MOPT(name) REGnames(string)]
	...
	mat `b'				= e(b)
	mat `V'				= e(V)
	loc varlist			: colnames `b'
	loc cons			"_cons"
	loc hascons			: list cons in regnames
	if !`hascons' & "`constant'" == "" {
		di as err "option first() incorrectly specified -- option noconstant required"
		exit 198
	}
	if "`constant'" != "" {
		if `: list cons in varlist' {
			di as err "option first() incorrectly specified -- constant found"
			exit 322
		}
		if `hascons' {
			loc regnames		: list regnames - cons
			mat `b'				= (`b', 0)
			mat `V'				= (`V', J(rowsof(`V'), 1, 0) \ J(1, colsof(`V')+1, 0)
		}
	}
	loc K1				: word count `varlist'
	loc K2				: word count `regnames'
	if `K1' != `K2' {
		di as err "option first() incorrectly specified -- incorrect number of coefficients"
		exit 322
	}
	if !`: list varlist == regnames' {
		if "`copy'" == "" {
			di as err "option first() incorrectly specified -- regressor names do not match"
		}
		else {
			di as txt "note: first-step and second-step regressor names do not match"
		}
	}
	mata: xtdpdgmm_init_first(`mopt', st_matrix("`b'"), st_matrix("`V'"))
end
...
*==================================================*
*** version history ***
* version 2.7.2  17jul2025  option wmatrix(residuals) replaced by option first()
* version 2.7.1  14jul2025  bugs fixed with option wmatrix(residuals)
* version 2.7.0  04jul2025  option wmatrix(residuals) added
* version 2.6.9  03apr2025  bug fixed with option overid
* version 2.6.8  05dec2024  option vce(robust, dc) not available with non-default weighting matrix and option overid
* version 2.6.7  11jul2024  postestimation command estat serialpm superseded by standalone command xtdpdserial
* version 2.6.6  07mar2024  bug fixed with estat serialpm after xtdpdgmm with option nl()
* version 2.6.5  06mar2024  option difference required with option order() of estat serialpm; bug fixed with insufficient observations for estat serialpm
* version 2.6.4  14jul2023  bug fixed with option difference of estat serialpm
* version 2.6.3  26apr2023  bug fixed with option cugmm; bug fixed with estat serialpm
* version 2.6.2  03aug2022  options difference, collapse, and order() added for estat serialpm
