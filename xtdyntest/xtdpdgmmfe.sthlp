...
{p 4 6 2}
You must {cmd:xtset} your data before using {cmd:xtdpdgmmfe}; see {helpb xtset:[XT] xtset}.{p_end}
{p 4 6 2}
...
In combination with option {opt orthogonal}, the maximum lag used is {it:#}-1. This ensures that the same number of instruments is used with or without option {opt orthogonal}.
...
When adding further instruments with option {cmd:iv()} or {cmd:gmmiv()}, the suboption {opt m:odel}{cmd:(}{it:{help xtdpdgmm##options_spec:model_spec}}{cmd:)} should be specified because {cmd:xtdpdgmmfe} changes the default model.
