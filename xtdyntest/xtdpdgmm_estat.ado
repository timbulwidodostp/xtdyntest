...
program define xtdpdgmm_estat_serialpm, rclass		// undocumented since version 2.6.7
	version 13.0
	syntax [, Difference Collapse FULLCollapse noForward Lagrange(numlist max=2 int miss >1) FS(numlist max=2 int miss >0) Order(numlist asc int >0)]

	di as txt "note: {bf:estat serialpm} is superseded by the new command {bf:{stata help xtdpdserial:xtdpdserial}}"
	tempvar smpl dsmpl e
	qui gen byte `smpl' = e(sample)
	...
	loc maxlag			= (r(max) - r(min)) / `_dta[_TSdelta]' - ("`difference'" != "")
	if "`lagrange'" != "" {
		if "`order'" != "" {
			di as err "options lagrange() and order() may not be combined"
			exit 184
		}
		if "`fs'" != "" {
			di as err "options lagrange() and fs() may not be combined"
			exit 184
		}
		...
		if "`order'" != "" {
			di as err "options fs() and order() may not be combined"
			exit 184
		}
		if "`difference'" != "" {
			di as err "options fs() and difference may not be combined"
			exit 184
		}
		if "`forward'" != "" {
			di as err "options fs() and noforward may not be combined"
			exit 184
		}
		if "`fullcollapse'" != "" {
			di as err "options fs() and fullcollapse may not be combined"
			exit 184
		}
		gettoken lag1 lag2 : fs
		if `lag1' == . {
			loc lag1			= 1
		}
		if "`lag2'" == "" {
		...
		if `lag1' > `maxlag' - 2 | `lag2' > `maxlag' | `lag2' - `lag1' < 2 {
			di as err "fs() invalid -- invalid numlist has elements outside of allowed range"
			exit 125
		}
		loc lag2			: list retok lag2
		loc forward			"noforward"
	}
	else if "`order'" == "" {
		if `maxlag' < 2 {
			error 2001
		}
		loc lagrange		"2 / `maxlag'"
	}
	if "`forward'" == "" & "`fullcollapse'" != "" {
		di as err "options noforward and fullcollapse may not be combined"
		exit 184
	}
	if "`difference'" != "" {
		if "`fullcollapse'" != "" {
			di as err "options difference and fullcollapse may not be combined"
			exit 184
		}
		if "`order'" != "" {
			loc lagrange		: subinstr loc order " " ",", all
			if max(., `lagrange') > `maxlag' {
				di as err "option order() incorrectly specified -- outside of allowed range"
				exit 125
			}
			if "`difference'" != "" & min(., `lagrange') == 1 {
				di as err "option order() incorrectly specified -- outside of allowed range"
				exit 125
			}
		}
		...
		di as err "option order() not allowed without option difference"
		exit 198
	}
	else {
		loc forward			= ("`forward'" == "")
		...
		if "`fs'" == "" {
			foreach j of num `lagrange' {
				if "`var'" != "`smpl'" {
					...
					loc indeplags		"`indeplags' L`j'`difference'.`smpl'"
				}
			}
			if `forward' {
			...
			if "`var'" != "`smpl'" {
				...
				fvrevar F`lag1'S`lag2'.`var' if `smpl'
				loc indeplags		"`indeplags' `r(varlist)'"
			}
			else {
				loc indeplags		"`indeplags' F`lag1'S`lag2'.`smpl'"
			}
		}
	}
	...
	mata: xtdpdgmm_serialpm(`e(mopt)', "`e'", "`elags'", "`indepvars'", "`indeplags'", "`scorevars'", "`_dta[_TSpanel]'", "`smpl'", "`dsmpl'", "`V'", ("`collapse'" != "")+2*("`fullcollapse'" != ""))
	loc chi2			= r(chi2)
	loc df				= r(df)
	loc p				= chi2tail(`df', `chi2')
	di _n as txt "Portmanteau test for autocorrelation of the" _c
	if "`difference'" == "" {
		di " level residuals"
	}
	else {
		di " first-differenced residuals"
	}
	di _n as txt "H0: no autocorrelation" _c
	if "`order'" != "" {
		di " of order `lagrange'" _c
	}
...
program define xtdpdgmm_estat_overid, rclass
	...
	if "`e(estimator)'" != "cugmm" & !("`e(estimator)'" == "igmm" & e(converged)) {
		...
		loc p_u				= cond(`J_u' >= 0, chi2tail(`df', `J_u'), .)
		if `miss_u' {
			di as txt "note: assumptions not satisfied" _c
		}
		else if `df' == 0 {
