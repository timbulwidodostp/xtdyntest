...
program define xtdpdgmm_p_iv, rclass
	...
	if !`constant' {
		loc indepvars		: subinstr loc indepvars "o._cons" "", w c(loc constant)
	}
...
program define xtdpdgmm_p_scores, rclass
	...
	if `wc' == 1 & e(steps) == 2 {
		loc wmatrix			"`e(wmatrix)'"
		gettoken wmatrix first : wmatrix
		if "`wmatrix'" == "first" {
			if "`: list retok first'" == "." {
				di as err "first-step estimation results not found"
				exit 301
			}
			tempname xtdpdgmm_e
			...
			forv k = 1 / `K1' {
				tempvar `score1'`k'
				loc scorevars1 "`scorevars1' `score1'`k'"
			}
			predict double `score1'* if `touse', score
			if _rc {
				di as err "first-step estimation command `e(cmd)' does not provide scores"
				exit 321
			}
			if `K1' == `K' - 1 {
				tempvar `score1'`K'
				loc scorevars1 "`scorevars1' `score1'`K'"
				qui gen byte `score1'`K' = 0 if `touse'
			}
			_est unhold `xtdpdgmm_e'
		}
	}
	mata: xtdpdgmm_score(`e(mopt)', "`scorevars'", "`smpl'", `wc', "`scorevars1'")
	if e(df_r) < . {
		if e(N_clust) < . {
